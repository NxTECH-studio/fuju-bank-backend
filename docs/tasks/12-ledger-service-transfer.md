...
User → User のふじゅ〜送金を処理する Service Object を実装する。
残高不足時は `InsufficientBalanceError` を raise し、トランザクション全体を rollback する。

## 背景・目的

- PayPay 的な User 間送金を MVP で提供。
...
class Ledger::Transfer
  ...
  def call
    ...
    raise BankError.new(code: "VALIDATION_FAILED", message: "cannot transfer to self") if @from_user.id == @to_user.id
...
- `spec/services/ledger/transfer_spec.rb`
  ...
  - 異常系: `from_user == to_user` → `BankError(VALIDATION_FAILED)`
  - 異常系: `amount <= 0` → `BankError(VALIDATION_FAILED)`
  - 並行性: 同一 User に対する同時 Transfer でも整合性が崩れない（`lock!` のスモーク確認。厳密テストは手動 or test-prof）

## 技術的な補足

- 残高不足時は `InsufficientBalanceError` を raise することでトランザクション全体が rollback され、
  DB 側の CHECK 制約（`balance_non_negative_for_user`）に到達しない。CHECK は「最後の砦」。
