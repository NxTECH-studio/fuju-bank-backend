# 14: GET /artists/:id エンドポイント（残高取得）

> 依存: #01, #09

## 概要

Artist 情報と残高（`balance_fuju`）を返す API。作家 HUD の起動時・残高画面で使う。
...
- **変更対象**:
  - `app/controllers/artists_controller.rb`（#13 で作成したものに `show` アクション追加）
  - `spec/requests/artists_spec.rb`（#13 と同ファイル）
- **破壊的変更**: なし
- **外部層への影響**: 新規エンドポイント公開
...
1. コントローラに `show` を追加
   ```ruby
   def show
     artist = Artist.includes(:account).find(params[:id])
     render(json: serialize(artist))
   end
   ```
2. routes は #13 で `resources :artists, only: [:create, :show]` に含まれる
3. Request spec
   ```ruby
   describe "GET /artists/:id" do
     ...
     it "returns balance_fuju" do
       ...
       expect(response.parsed_body).to include("id" => artist.id, "balance_fuju" => 0)
     end
