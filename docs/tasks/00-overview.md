...
鑑賞者がアート作品の前で滞留し・視線を向け「魂を削られた」時間を、ふじゅ〜としてユーザーに還元する
というコンセプトを支える中央台帳。
...
1. `User` / `Artifact` / `Account` の CRUD 基盤
2. **発行（mint）**: `Artifact` から `User` への新規ふじゅ〜発行
3. **送金（transfer）**: `User` → `User` のふじゅ〜移動
4. 残高照会 / 取引履歴 API
5. `UserChannel` による `credit` イベントのリアルタイム配信
...
```mermaid
erDiagram
    User ||--|| Account : "has_one (kind: user)"
    ...
    Account {
        bigint id PK
        bigint user_id FK "nullable (system 発行口は NULL)"
        string kind "enum: system_issuance / user"
...
- `after_create` で対応する `Account`（`kind: "user"`）を自動作成（`User` と `Account` は 1:1）。
...
- 勘定口座の抽象化。`kind` enum で役割を区別。
  - `system_issuance`: ふじゅ〜の発行源（マイナス残高許容）。`user_id` は NULL。システム初期化時に1行 seed。
  - `user`: User 口座（マイナス残高禁止、`balance_fuju >= 0` CHECK 制約）。`user_id` は必須。
...
`POST /ledger/mint` で呼ばれる。`artifact_id` → `user_id` に N ふじゅ〜を発行する。

```
ledger_transaction: kind=mint, artifact_id=X, idempotency_key=...
  entries:
    - account_id=<system_issuance>, amount=-N   (貸方: 発行口から出る)
    - account_id=<user X's account>, amount=+N (借方: User に入る)
```

`accounts.balance_fuju` 更新:
- `system_issuance.balance_fuju -= N`（負の値になってよい。CHECK 制約対象外）
- `user_account.balance_fuju += N`

#### Transfer（送金）

`POST /ledger/transfer` で呼ばれる。User A → User B に N ふじゅ〜を送る。
...
| Method | Path | 用途 | 認証 |
|---|---|---|---|
| `POST` | `/users` | User 作成 | TODO: 将来拡張 |
| `GET` | `/users/:id` | User 情報 + 残高取得 | TODO: 将来拡張 |
| `GET` | `/users/:id/transactions` | 取引履歴（mint / transfer 統合） | TODO: 将来拡張 |
| `POST` | `/artifacts` | Artifact 作成 | TODO: 将来拡張 |
| `GET` | `/artifacts/:id` | Artifact 情報 | TODO: 将来拡張 |
| `POST` | `/ledger/mint` | 発行（マイニング層から） | TODO: 将来拡張 |
| `POST` | `/ledger/transfer` | 送金（User → User） | TODO: 将来拡張 |
...
または単純なリソースレスポンス（`render json: user_hash`）。
...
- **接続**: クライアント（作家 HUD PWA）は `user_id` をクエリ or subscribe params で渡す。
- **broadcast 先**: mint / transfer の **受け手 User の Channel** に push。
- **MVP の認証**: `user_id` のみで identify（簡易）。将来 `public_key` 署名検証を追加する TODO を残す。
...
- `from_user_id`: transfer の場合のみ。mint は `null`。

### 6.3 broadcast フック

`Ledger::Mint` / `Ledger::Transfer` サービスが成功したら、**受け手 User の Account に紐づく User** に
`UserChannel.broadcast_to(user, payload)` する（タスク `20-broadcast-on-credit.md`）。
...
| # | タスク | 依存 |
|---|---|---|
| 04 | [users テーブル](./04-add-users-table.md) | #02 |
...
| # | タスク | 依存 |
|---|---|---|
| 09 | [User モデル + Account bootstrap](./09-user-model-and-account-bootstrap.md) | #04, #06 |
...
| # | タスク | 依存 |
|---|---|---|
| 13 | [POST /users](./13-users-controller-create.md) | #01, #09 |
| 14 | [GET /users/:id](./14-users-controller-show-balance.md) | #01, #09 |
...
| 18 | [GET /users/:id/transactions](./18-transactions-list-endpoint.md) | #01, #09, #11, #12 |

### Phase 5: リアルタイム配信

| # | タスク | 依存 |
|---|---|---|
| 19 | [UserChannel 骨組み](./19-user-channel-skeleton.md) | #09 |
...
- `#19` / `#20` は User モデル（#09）と Ledger サービス（#11, #12）の完成後に実装。
