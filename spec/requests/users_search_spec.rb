...
RSpec.describe "GET /users/search", type: :request do
  ...
  context "正常系" do
    ...
    it "200 と { users: [...] } を返し、各要素は id / public_id / name / icon_url のみ" do
      get("/users/search", params: { q: "アリス" })
      ...
      parsed["users"].each do |hit|
        expect(hit.keys).to match_array(%w[id public_id name icon_url])
    ...
    it "ヒット 0 件は 200 + 空配列を返す（404 ではない）" do
      get("/users/search", params: { q: "存在しない" })
  ...
  context "limit" do
    let!(:targets) { create_list(:user, 25, name: "サンプル太郎") }

    it "デフォルト 10 件返す" do
      get("/users/search", params: { q: "サンプル" })

      expect(response.parsed_body["users"].size).to eq(10)
    end

    it "明示指定の limit を尊重する" do
      get("/users/search", params: { q: "サンプル", limit: 5 })

      expect(response.parsed_body["users"].size).to eq(5)
    end

    it "上限 (20) ちょうどは valid" do
      get("/users/search", params: { q: "サンプル", limit: 20 })
  ...
  context "SQL ワイルドカードのエスケープ" do
    let!(:plain) { create(:user, name: "100", public_id: "u_plain") }
    let!(:percent) { create(:user, name: "100%off", public_id: "u_percent") }
  ...
  context "バリデーション" do
    ...
    it "limit が 0 なら 400 VALIDATION_FAILED" do
      get("/users/search", params: { q: "アリス", limit: 0 })
    ...
    it "limit が 21 なら 400 VALIDATION_FAILED" do
      get("/users/search", params: { q: "アリス", limit: 21 })
    ...
    it "limit が非数値なら 400 VALIDATION_FAILED" do
      get("/users/search", params: { q: "アリス", limit: "abc" })
