...
RSpec.describe "Idempotent", type: :request do
  ...
  describe "正常系" do
    ...
    it "ヘッダ名は大文字小文字を区別しない（HTTP 規約）" do
      get "/testing_idempotent", headers: { "idempotency-key" => valid_key }
    ...
    it "ヘッダが空文字でも body に有効値があれば body にフォールバックする" do
      post "/testing_idempotent",
           params: { idempotency_key: valid_key },
  ...
  describe "異常系" do
    ...
    it "body params が空文字で 400 VALIDATION_FAILED を返す" do
      post "/testing_idempotent", params: { idempotency_key: "" }
