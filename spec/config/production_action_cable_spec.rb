...
RSpec.describe "config/environments/production.rb (Action Cable)" do # rubocop:disable RSpec/DescribeClass
  ...
  it "allows the wss://api.fujupay.app origin" do
    match = source.match(/config\.action_cable\.allowed_request_origins\s*=\s*(\[[^\]]+\])/m)
    expect(match).not_to be_nil, "allowed_request_origins assignment must be present"

    origins = eval(match[1]) # rubocop:disable Security/Eval -- evaluating our own source so the test reflects real Regexp behavior
